...
interface StatsCardProps {
  data?: Awaited<ReturnType<typeof GetFormStats>>; //Es el tipo del valor que se obtendría al esperar la promesa que devuelve GetFormStats. GetFormStats es el tipo de lo que se resuelve después de esperar la ejecución de GetFormStats
  loading: boolean;
}
...
function StatsCard({ title, value, icon, helperText, loading, className }: {
  ...
  <Card className={className}>
    <CardHeader className="flex flex-row items-center justify-between pb-2">
      <CardTitle className="text-sm font-medium text-mutedforeground">{title}</CardTitle>
      {icon}
    </CardHeader>
    <CardContent>
      ...
      <CardDescription className="flex items-center justify-between text-muted-foreground text-sm">
        ...
        {form.published && (
          <span className="flex items-center gap-2">
            ...
            <span>{form.submisions.toLocaleString()}</span>
          </span>
        )}
      </CardDescription>
    </CardHeader>
